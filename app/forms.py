...
class CustomerRegistrationForm(UserCreationForm ):
    ...
    class Meta:
        ...
        labels = {'email':'Email'}
        widgets = {'username':forms.TextInput(attrs={'class':'flow-control'})}

class LoginForm(AuthenticationForm):
    username = UsernameField(widget=forms.TextInput(attrs=
    {'autofocus':True,'class':'form-control'}))
    username = forms.CharField(Label=_("Password"),strip=False, widget=forms.PasswordInput(attrs=
...
class MyPasswordResetForm(PasswordResetForm):
    email = forms.EmailField(label=_("Email"), max_length=254,
     widget=forms.EmailInput(attrs={'autocompltete':'email','class':'form-control'}))
...
class CustomerProfileForm(forms.ModelForm):
    class Meta:
        model = Customer
        field = ['name','locality','city','state','zipcode']
